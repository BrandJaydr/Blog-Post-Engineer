( function($) {
	...
	$(document).on('click', '.bdpp-filter-btn', function() {
		var btn       = $(this);
		var bar       = btn.closest('.bdpp-filter-bar');
		var unique    = bar.data('unique');
		var gridWrap  = $('#bdpp-post-grid-' + unique);
		var gridInner = gridWrap.find('.bdpp-post-grid-inr-wrap');
		var cat       = btn.data('cat');
		var conf      = bar.data('conf');

		if (btn.hasClass('active') && gridWrap.hasClass('bdpp-filter-loading')) {
			return false;
		}

		bar.find('.bdpp-filter-btn').removeClass('active');
		btn.addClass('active');
		
		gridInner.css('opacity', '0.5');
		gridWrap.addClass('bdpp-filter-loading');

		// Premium micro-interaction: hide pagination when viewing a filtered category
		if (cat !== '') {
			gridWrap.find('.bdpp-paging, .bdpp-post-load-more, .navigation').hide();
		} else {
			gridWrap.find('.bdpp-paging, .bdpp-post-load-more, .navigation').show();
		}

		var data = {
			action: 'bdpp_filter_posts',
			nonce:  Bdpp.filter_nonce,
			cat:    cat,
			conf:   conf
		};
		...
		$.post(Bdpp.ajax_url, data, function(res) {
			if (res.success) {
				...
				gridInner.html('<div class="bdpp-no-posts">' + (res.data || 'Error loading posts') + '</div>');
			}
			gridInner.css('opacity', '1');
			gridWrap.removeClass('bdpp-filter-loading');
		}).fail(function() {
			gridInner.html('<div class="bdpp-no-posts">Error loading posts. Please try again.</div>');
			gridInner.css('opacity', '1');
			gridWrap.removeClass('bdpp-filter-loading');
		});
	});
